    ...
    var body: some View {
        NavigationStack {
            if let podcasts = folder.podcasts {
                NavigationLink {
                    FolderRecentEpisodesView(folder: folder)
                } label: {
                    HStack {
                ...
                List {
                    ForEach(podcasts, id: \.title) { podcast in
                        NavigationLink {
                            FolderPodcastListView(collectionId: podcast.collectionId, podcastTitle: podcast.title)
                            // TODO: create new label, only pictures shows as grid
                        } label: {
                            HStack {
